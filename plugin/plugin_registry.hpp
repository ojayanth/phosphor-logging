 ...
 * used to construct runtime plugin instances from
 * creation descriptors.
 */
using PluginCreateFunc = std::function<std::unique_ptr<Plugin>(
    const PluginContext&, const plugin::Descriptor&)>;
...
class PluginRegistry
{
  public:
     ...
     *
     * Looks up the registered callback associated with
     * the supplied descriptor type and invokes it.
     *
     * @param[in] context Plugin creation context.
     * @param[in] descriptor Plugin creation descriptor.
     *
     * @return Plugin instance or nullptr if the plugin
