...
This directory contains scripts used to generate the AMD Event Log (AEL) reverse lookup table (LUT) and associated unit tests at build time.

The code generation process converts JSON-based LUT definitions into C++ source artifacts used by the AEL decoder.
...
These scripts are invoked automatically through Meson `custom_target` definitions and are not intended to be run manually during normal builds.
